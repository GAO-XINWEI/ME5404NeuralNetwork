...
%% b) Normalization (Batch Norm)
dataset = [img_train, img_test];
value_mean = mean(mean(dataset));
value_var = mean(std(dataset,1));%1:mean(std(dataset,1)).^2 %2:mean(std(dataset,1))
...
xlabel('Epoch'), ylabel('Accuracy'), title('Single layer perceptron (Batch Norm)');
